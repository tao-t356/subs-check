...
func makeRequest(client httpClient, method, url string) ([]byte, error) {
	...
	req.Header.Set("Authorization", fmt.Sprintf("Bearer %s", config.Current().MihomoApiSecret))
...
func UpdateSubs() {
	if config.Current().MihomoApiUrl == "" {
...
func getVersion(client httpClient) (string, error) {
	url := fmt.Sprintf("%s/version", config.Current().MihomoApiUrl)
...
func getNeedUpdateNames(client httpClient) ([]string, error) {
	url := fmt.Sprintf("%s/providers/proxies", config.Current().MihomoApiUrl)
	body, err := makeRequest(client, http.MethodGet, url)
	if err != nil {
		return nil, err
	}
...
func updateSubs(client httpClient, names []string) error {
	for _, name := range names {
		url := fmt.Sprintf("%s/providers/proxies/%s", config.Current().MihomoApiUrl, name)
